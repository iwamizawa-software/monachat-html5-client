...
***************/
var LANGUAGE;
var SHOW_POPUP;                     // 0 - NONE, 1 - SOME, 2 - ALL
var IS_LOADING_MAIN  = false;       // Flag to set main room loader
var KEEP_LOOKING_ID  = false;       // lookat id
var LOOP_INTERVAL_ID = false;       // /loop interval id
var PID              = 'MASTER';    // IPC PID
...
try
    {
        config         = jsonfile.readFileSync('./app/config.json');
        login_data     = jsonfile.readFileSync('./app/config.json');
        trip_list      = jsonfile.readFileSync('./app/trip.json');
        character_list = jsonfile.readFileSync('./app/assets/js/character_list.json');
        
        LANGUAGE    = config.language;
        SOUND_ON    = config.sound;
        SHOW_POPUP  = config.popup ? 1 : 0;
...
ipcRenderer.on('slave_msg', function(e, msg)
    {
        console.log('Message from slave: ', msg);
    });
...
ipcRenderer.on('ctx_menu', function(e, args)
    {
        console.log(e);
        console.log(args);
        ...
        /* Room context menu */
        msg == 'next'       ? next_room()                                                               :
        msg == 'prev'       ? previous_room()                                                           :
        msg == 'profile'    ? add_profile()                                                             :
        msg == 'default'    ? set_default()                                                             :
        msg == 'random'     ? set_random()                                                              :
        msg == 'nanashi'    ? set_nanashi()                                                             :
        msg == 'invisible'  ? set_invisible()                                                           :
        msg == 'anonymous'  ? set_anonymous()                                                           :
        msg == 'up_image'   ? util.upload_image(args[0])                                                :
        msg == 'up_file'    ? util.upload_file(args[0])                                                 :
        msg == 'tgl_sound'  ? (SOUND_ON = !SOUND_ON)                                                    :
        msg == 'show_popup' ? (SHOW_POPUP = (SHOW_POPUP+1) % 3)                                         :
        msg == 'new'        ? new_instance( { n: 1, room: session.room(), prof: session.get_data() } )  :
        
        /* User context menu */
        msg == 'user_data'  ? print_user_data(args[0])                                                  :
        msg == 'names'      ? search_trip('id', args[0])                                                :
        msg == 'copy'       ? copy(args[0])                                                             :
        msg == 'ignore'     ? ignore(args[0])                                                           :
        msg == 'mute'       ? mute(args[0])                                                             :
        msg == 'search_name'? search_trip('namerecursive', user[args[0]].name)                          :
        msg == 'stalk'      ? toggle_stalk(args[0])                                                     :
        msg == 'repeat'     ? toggle_repeat(args[0])                                                    :
...
**/
window.eval = global.eval = function() { console.log("No can do."); };
...
/**
* Pipe console output to log
**/
var console_log = console.log;
console.log = function(args)
    {
        fs.appendFile('./app/log.txt', args,   () => '');
        fs.appendFile('./app/log.txt', '\r\n', () => '');
...
*************************/
const SCREEN_MIN_X = 60;
const SCREEN_MAX_X = 662;
const SIGNAL_MIN_X = 30;
const SIGNAL_MAX_X = 704;
...
function save_config()
    {
        ...
        *********************/
        ...
        jsonfile.writeFile('./app/config.json', config, function(err)
            {
...
function save_trip()
    {
        ...
        *********************/
        if(Object.keys(trip_list).length > 0)
            {
                jsonfile.writeFile('./app/trip.json', trip_list, function(err)
                    {
                        if(err) { console.log(err); }
                    });
            }
...
function load_bots()
    {
        ...
        try        { bot_list = fs.readdirSync('./app/assets/js/bots').filter( (file) => file.match(/\.js$/) ); }
        catch(err) { log(['Couldn\'t load bots.']); }
...
function open_with_browser(e, url)
    {
        ...
        ipcRenderer.send('open_browser', url);
    }
...
*/
                ...
                if(config.log_text_background) { $(text_el).css('background-color', 'rgb('+r+', '+g+', '+b+')'); }
            }
...
function print_user_data(id, n_registered, trip_len)
    {
...
function format_log(type, args)
    {
        ...
        var id, stat, room, ihash, r, g, b, cmt;
        ...
        if(type == 'enter')
        ...
        else if(type == 'com')
            {
                ...
                [r, g, b] = [user[id].r, user[id].g, user[id].b];
                ...
                ***************/
                ...
                if(validator.isURL(cmt))
                    {
                        cmt_el = log_a_el(cmt);
                        
                        nbsp_el = log_nbsp_el(1);
                        
                        var open_url_button = $( document.createElement('BUTTON') )
                            .addClass('open_url_button_log')
                            .text('URL読込')
                            .on('click', function()
                                {
                                            ...
                                            success: function(msg, data, jqXHR)
                                                {
                                                    ...
                                                    ************************************************/
                                                    if(/^image/.test(type) && size/1024 < 20000)
                                                            {
                                                                var img_el  = log_img_el(cmt);
                                                                
                                                                open_url_button.replaceWith('<br><br>', img_el, '<br>');
                                                            }
                                                    else if(type == 'video/webm' && size/1024 < 200000)
                                                        {
                                                            ...
                                                            open_url_button.replaceWith('<br><br>', video_el, '<br>');
                                                        }
                                                    else if(type == 'audio/webm' && size/1024 < 20000)
                                                        {
                                                            var audio_el = $(document.createElement('audio'))
                                                                .attr('src', cmt)
                                                                .attr('controls', '');
                                                            
                                                            open_url_button.replaceWith('<br><br>', audio_el, '<br>');
                                                        }
                                                    else if(host == 'www.youtube.com' || host == 'youtu.be')
                                                        {
                                                            ...
                                                            open_url_button.replaceWith('<br><br>', iframe_el, '<br>');
                                                        }
                                                                    ...
                                                                    success: function(msg, data, jqXHR)
                                                                        {
                                                                            ...
                                                                            open_url_button.replaceWith(text_el);
                                                                        }
                                                                });
                                                        }
                                                },
                                            ...
                                            error: function(err)
                                                    {
                                                        ...
                                                        open_url_button.replaceWith( error_el );
                                                    }
                                        });
                                });
                        
                        log([time_el, user_el, two_dot, cmt_el, nbsp_el, open_url_button]);
                    }
                else
                    {
...
function save_log()
    {
        ...
        /*******************************************************
        * Replace characters not accepted by windows filenames
        *******************************************************/
        ...
        ***************************************/
        fs.readdir('./app/log', function(err)
            {
                if(err)
                    {
                        fs.mkdirSync('log', (err) => console.log('Couldn\'t create log folder.') );
                    }
                
                fs.writeFile('./app/log/'+date+'.txt', text, function(err)
                    {
                        if(err) { log(['Error: Couldn\'t save log.']); }
                    });
            });
...
function search_log(word)
    {
        var logs = fs.readdir('log', function(err, logs)
            {
                if(err) { return; }
                
                logs = logs.filter( (filename) => filename.match(/\.txt$/) );
                ...
                for(var i = 0; i < logs.length; i++)
                    {
                        ...
                        filename = __dirname + '/log' + logs[i];
                        
                        try        { file = fs.readFileSync(filename, 'utf8').split('\n'); }
                        catch(err) { log(['Couldn\'t load logs.']); return; }
...
function check_transparency(id, e)
    {
        ...
        if(character_list[character] === undefined) { character = 'mona'; }
        ...
        img.onload = function()
            {
                ...
                *******************************/
                var user_data = $(document.createElement('TEXT'))
                    .addClass('user_data')
                    .html(
                            (user[id].name || 'nanashi').replace(/[<>"'&]/g, function (s) { return '&#' + s.charCodeAt(0) + ';'})
                            + '<br>' + WHITE_TRIP_SYM
...
function add_comment_div(id, cmt)
    {
        ...
        **********************************************/
        ...
        var div_el = $(document.createElement('DIV'))
            ...
            .on('click', () => ipcRenderer.send('copy_clipboard', cmt) );
...
function show_user_context_menu(id)
    {
        ...
        var user_data = format_user_data(id, 3);
            

        ipcRenderer.send('open_user_context_menu',
            {
                user_data: user_data,
                names: names,
                id: id,
                is_stalked: is_stalked(id),
                is_repeated: is_repeated(id)
            });
    }
...
function show_command_context_menu()
    {
        ...
        ******************************/
        ...
        ipcRenderer.send('open_room_context_menu',
            {
                prev_room: prev,
                next_room: next,
                sound_on: SOUND_ON,
...
function change_server(server)
    {
        ...
        if(!res) { format_log('error', ['Server name not valid.']); }
        else     { clear_screen(); }
    }
...
function popup(title, msg)
    {
        ipcRenderer.send('popup', {title: title, msg: msg});
    }
...
function signal_handler(msg)
    {
        ...
        ************************/
        if(msg[0] == '+')
            ...
            {
                try        { xml = new xmldoc.XmlDocument(msg); }
                catch(err) { console.log('Could not parse XML:', err); return; }
                        ...
                        if(!is_muted(id) && !is_filtered(id))
                            {
                                ...
                                if(SOUND_ON && session.room() != 'main')
                                    {
                                        play('./app/assets/sound/enter.wav');
                                    }
                            }
                    }
                        ...
                        if(!is_muted(id) && !is_filtered(id))
                            {
                                ...
                                if(SOUND_ON) { play('./app/assets/sound/comment.wav'); }
...
function command_handler(com)
    {
        ...
        if     (com[0] == 'name')                             { session.name(full_arg);                 }
        ...
        else if(com[0] == 'enqueuecomment')                   { session.enqueue_comment(full_arg);      }
        ...
        else if(com[0] == 'loop')
            {
                if(com.length == 1)
                    {
                        ...
                        LOOP_INTERVAL_ID = false;
                        
                        return;
                    }
                
                if(LOOP_INTERVAL_ID) { clearInterval(LOOP_INTERVAL_ID); }
                ...
                LOOP_INTERVAL_ID = setInterval( () => command_handler(com), time );
            }
            ...
            {
                options = { n: 1, room: session.room(), prof: session.get_data() };
...
function process_arguments()
    {
        ...
        for(var i = 2; i < ARGV.length; i += 2)
            {
                if(ARGV[i] == 'PID') { PID = ARGV[i+1]; }
                else                 { login_data[ARGV[i]] = ARGV[i+1]; }
            }
    }
    ...
    $('#config_menu_client_add_config').on('click', function()
        {
            dialog.showOpenDialog
                (
                    function(path)
                        {
                            jsonfile.readFile(path[0], function(err, file)
                                {
            ...
            if(e.key == 'Enter' && e.target.value !== '')
                {
                    ...
                    if(text[0] == '/')
                        ...
                        {
                            if( text.length > 50 && validator.isURL(text) && !(text.includes('youtube') || text.includes('youtu.be')) )
                                {
                                    tinyurl(text);
                                }
                            else
