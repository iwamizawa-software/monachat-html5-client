...
function reconnect(s)
    {
        ...
        *************/
        ...
        if(this._proxy)
            {
                if(this._site == 1 && config && config.download_proxy_always) { this.proxy_list = []; }
                this.connect();
            }
        else
            {
        ...
        if(this.client.writable)
            {
                ...
                try        { (msg == 'MojaChat\0') || new xmldoc.XmlDocument(msg);     }
                catch(err) { console.log('Invalid XML structure: ', err, msg); return; }
